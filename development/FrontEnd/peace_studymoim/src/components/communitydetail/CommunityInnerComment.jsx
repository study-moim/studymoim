...
export default function CommunityInnerComment({ child }) {
  ...
  if (info && info.userId === child.user.userId) {
    setIsMine(true);
  }

  const dateBase = new Date(child.publishTime);
  const date = dateBase.toString().substring(11, 24);

  return (
    <>
      <div className="flex flex-col border justify-end items-end w-full m-[40px]">
        <div className="w-full">
          {/* 댓글 맨위 */}
          <div className="flex flex-row justify-between">
            <div className="flex justify-start items-end relative gap-px m-2">
              <img
                className="w-10"
              ...
              />
              <Link
                to={`/mypage/${child.user.userId}`}
                className="hover:text-cyan-700 text-black hover:scale-105"
                state={{clickWho: child.user.userId}}
              >
            ...
            <button
              className={
                !isMine
                  ? "invisible"
                  : "h-9  p-2 rounded-[10px] bg-[#F24E1E] text-[12px] font-bold text-center m-[5px] text-white hover:bg-[#f24f1ee8] hover:scale-95"
              }
            >
              삭제하기
            </button>
          ...
          {/* 대댓글 내용 */}
          <div className="flex w-full justify-start p-[30px] text-[15px] text-center text-black">
            {child.content}
          </div>
        </div>
      </div>
