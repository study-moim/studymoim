              ...
              {requestMembers.map((member) => {
                return (
                  <div key={member.requestId}>
                    <img className="flex-grow-0 flex-shrink-0 w-2/12" src="" />
                    ...
                    <button
                      onClick={() =>
                        fetch(
                          `http://${API_SERVER}/api/v1/study/${propData.studyId}/request/${member.requestId}`,
                          {
                            ...
                            body: JSON.stringify({userId: member.user.userId, requestStatus: 1 }),
                          }
                        ).then((res) => {
                          if (res.ok) {
                            location.reload();
                      ...
                      className="flex-grow-0 flex-shrink-0 text-2xl font-bold text-center text-white gap-2.5 p-2.5 rounded-[10px] bg-[#bd6ffc]"
                    >
                      수락
                    </button>
                    <button
                      onClick={() =>
                        fetch(
                          `http://${API_SERVER}/api/v1/study/${propData.studyId}/request/${member.requestId}`,
                          {
                            ...
                            body: JSON.stringify({userId: member.user.userId, requestStatus: 2 }),
                          }
                        ).then((res) => {
                          if (res.ok) {
                            location.reload();
