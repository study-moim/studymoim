...
export default function CourseMainRoot() {
  ...
  const getPage = async () => {
    ...
    if (params.key == "word") {
      ...
      resp = await fetch(
        `http://${API_SERVER}/api/v1/course/category/${params.word}?page=${params.page}&size=${params.size}`
      );
      data = await resp.json();
    } else {
      resp = await fetch(
        `http://${API_SERVER}/api/v1/course?key=title&word=&page=${params.page}&size=${params.size}`
      );
      data = await resp.json();
    }
    setPage(data);
